...
const key string = "owejf)oiejl;zjsdfo*QS#sJ"
...
func Post(w http.ResponseWriter, r *http.Request) {
	...
	insertComplete := PutSecret(encryptedSecret, token, expireDate, numViews)
	fmt.Println(insertComplete)
	URLString := "https://" + r.Host + "/secret?token=" + token
...
func EncryptString(key, text string) string {
	...
	return fmt.Sprintf("%0x", ciphertext)
...
func GetSecret(token string, key string) string {
	...
	for foundsecret.Next() {
		...
		fmt.Println(secret)
		secret = DecryptString(key, secret)
	}
	return secret
}
